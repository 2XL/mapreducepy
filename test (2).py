...
# @profile
@fn_timer
def map_reduce(distributionType, clusterSize, logData, outputPath, pool,
               threadLoadMap, threadLoadCombine,  threadLoadReduce, threadBandwidthIn, threadBandwidthOut, reduceCounter):
    """
    ...
    combineStatistics = pool.map(Combiner, ((item, combined) for item in list))
...
@fn_timer
def map_reduce_uniform(distributionType, clusterSize, logData, outputPath, pool, threadLoadMap,
            threadLoadCombine, threadLoadReduce, threadBandwidthIn, threadBandwidthOut, reduceCounter):
    # print "map_reduce/Start! ",
    # print distributionType, clusterSize, outputPath
    ...
    for x in map_visitor:
        list.append(x[0])
    # list = map_visitor
    # list = [ map_visitor[x][0] for x in map_visitor ]
    # print ".....LIST----------"
    # print list
...
if __name__ == "__main__":
    ...
    for y in range (0, numberOfRuns):
        print(y),
        if uniform == 1:
            visitor_frequency = map_reduce_uniform(False, clusterSize[clusterSizeIndex], logFile, "file/out/", pool, threadLoadMap,
            threadLoadCombine, threadLoadReduce, threadBandwidthIn, threadBandwidthOut, reduceCounter);
            # parseFrequency(visitor_frequency, "file/log_uniform.txt")
        else:
            visitor_frequency = map_reduce(False, clusterSize[clusterSizeIndex], logFile, "file/out/", pool, threadLoadMap,
            threadLoadCombine, threadLoadReduce, threadBandwidthIn, threadBandwidthOut, reduceCounter)
            # parseFrequency(visitor_frequency, "file/log_non_uniform.txt")
        if profileMemory == 1:
