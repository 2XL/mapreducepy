...
def map_reduce(distributionType, clusterSize, testRun, logData, outputPath, pool, threadLoadMap, threadLoadReduce, threadBandwidthIn, threadBandwidthOut, mapCounter):
    ...
    # SHUTTLE/SORT
    combiner_visitor = Partition([(x[0]) for x in map_visitor])
...
if __name__ == "__main__":
    ...
    threadBandwidthOut = {}
    mapCounter = {}
    pool = multiprocessing.Pool(processes=clusterSize[x], )
    #with py_performance.timer.Timer() as t:
    ...
    for y in range (0, numberOfRuns):
        map_reduce(False, clusterSize[x], testRunsRandom, logFile, "file/out/", pool, threadLoadMap, threadLoadReduce,
                   threadBandwidthIn, threadBandwidthOut, mapCounter)
        if profileMemory == 1:
    ...
    if profileMemory == 1:
        ...
        print "Average memory used", totalMemory / numberOfRuns
        print "Min memory used", minMemory
        print "Max memory used", maxMemory
        print ">>>>>>>>>>>>>>> END", clusterSize[x]
        for id, dataLoad in threadBandwidthIn.items():
            threadBandwidthIn[id] /= (mapCounter[id] + numberOfRuns) # +number of runs bcs in map and reduce together
    ...
    print threadBandwidthIn
    print threadBandwidthOut
